...
function ensureV19(){
  ...
  /* v2.4.1: legacy project seed is creation-only. */
  if(!home){home={id:'p-home-caddie',name:'우리집캐디',desc:'수입 대비 지출 관리와 목표 달성을 캐디 피드백과 게임화로 돕는 가족 자산관리 앱. KOSIS 생활권 분석은 후속 버전.',progress:20,status:'Active',current:'Flutter v0.1 소스 생성·실행 대기',next:'실기기 확인 및 v0.1 피드백'};data.projects.push(home);}
...
/* ============================================================
   ...
   ============================================================ */
(function studioOSV241(){
  const V241='2.4.1';
  data.versionGuard241=data.versionGuard241||{projects:{},events:[]};
  const clone241=x=>JSON.parse(JSON.stringify(x??null));
  const nums241=v=>{const raw=String(v??'').trim().replace(/^[vVsS]\s*/,'');const m=raw.match(/\d+(?:\.\d+)*/);return m?m[0].split('.').map(n=>Number(n)||0):[0]};
  const compareVersion241=(a,b)=>{const A=nums241(a),B=nums241(b),n=Math.max(A.length,B.length);for(let i=0;i<n;i++){const x=A[i]||0,y=B[i]||0;if(x>y)return 1;if(x<y)return-1}return 0};
  const time241=v=>{const t=Date.parse(v||'');return Number.isFinite(t)?t:0};
  const findLocal241=p=>data.projects.find(x=>p?.name&&x.name===p.name)||data.projects.find(x=>p?.id&&x.id===p.id)||null;
  const dev241=local=>local?data.developmentRecords.find(x=>x.projectId===local.id):null;
  const historyMax241=name=>{let best='0.0.0';(data.patchImports||[]).filter(x=>x.project===name&&x.type!=='Daily Closing').forEach(x=>{if(compareVersion241(x.version,best)>0)best=String(x.version||best)});return best};
  const actualVersion241=patch=>{const local=findLocal241(patch?.project),d=dev241(local);return String(d?.currentVersion||local?.version||'0.0.0')};
  const knownVersion241=patch=>{const local=findLocal241(patch?.project),actual=actualVersion241(patch),hist=historyMax241(local?.name||patch?.project?.name||'');return compareVersion241(hist,actual)>0?hist:actual};
  const guard241=patch=>{const incoming=String(patch?.projectVersion||patch?.development?.currentVersion||patch?.project?.version||'0.0.0'),known=knownVersion241(patch),actual=actualVersion241(patch),ck=compareVersion241(incoming,known),ca=compareVersion241(incoming,actual),key=patch?.project?.name||patch?.project?.id||'Unknown',meta=data.versionGuard241.projects[key]||{},it=time241(patch?.generatedAt),lt=time241(meta.lastAppliedGeneratedAt);if(ck<0)return{action:'BLOCKED',reason:`최신 기준 ${known}보다 이전 버전 ${incoming}`,incoming,known,actual};if(ck===0&&ca<0)return{action:'RECOVERY',reason:`실제 상태 ${actual} → 최신 기준 ${incoming} 복구`,incoming,known,actual};if(ck===0&&ca===0){if(it&&lt&&it<lt)return{action:'BLOCKED',reason:'같은 버전의 더 오래된 Patch',incoming,known,actual};if(it&&lt&&it===lt)return{action:'SKIPPED',reason:'동일 버전·동일 생성시각 Patch',incoming,known,actual};return{action:'SAME_VERSION',reason:'같은 버전 안전 Merge',incoming,known,actual}}return{action:'UPDATE',reason:`${actual} → ${incoming}`,incoming,known,actual}};
  const mergeOne241=(list,item)=>{const key=String(item?.id||item?.name||'');let cur=key?list.find(x=>String(x?.id||x?.name||'')===key):null;if(!cur){list.unshift(clone241(item));return'added'};if(JSON.stringify(cur)===JSON.stringify(item))return'skipped';Object.assign(cur,clone241(item));return'merged'};
  const mergeList241=(list,items)=>{const r={added:0,merged:0,skipped:0};(items||[]).forEach(x=>{const k=mergeOne241(list,x);r[k]=(r[k]||0)+1});return r};
  const mergeAsset241=(item,patchVersion)=>{const key=String(item?.id||item?.name||''),cur=key?(data.digitalAssets||[]).find(x=>String(x?.id||x?.name||'')===key):null;if(!cur){data.digitalAssets.unshift(clone241(item));return'added'}const incoming=item?.version||patchVersion||'0.0.0',existing=cur?.version||'0.0.0';if(compareVersion241(incoming,existing)<0)return'blocked';Object.assign(cur,clone241(item));return compareVersion241(incoming,existing)>0?'updated':'merged'};
  const logGuard241=(patch,state)=>{const key=patch?.project?.name||patch?.project?.id||'Unknown',now=new Date().toISOString();data.versionGuard241.events.unshift({project:key,patchId:patch.patchId,version:patch.projectVersion,status:state.action,reason:state.reason,date:now});data.versionGuard241.events=data.versionGuard241.events.slice(0,100);if(['UPDATE','RECOVERY','SAME_VERSION'].includes(state.action))data.versionGuard241.projects[key]={version:patch.projectVersion,lastAppliedGeneratedAt:patch.generatedAt||now,lastPatchId:patch.patchId,updatedAt:now}};
  const historyRow241=(p,s)=>({patchId:`${p.patchId}::${s.action}::${Date.now()}`,sourcePatchId:p.patchId,type:p.patchType,project:p.project?.name||'-',version:p.projectVersion,date:new Date().toLocaleString('ko-KR'),status:s.action,reason:s.reason,counts:{rules:(p.rules||[]).length,assets:(p.assets||[]).length,experiences:(p.experiences||[]).length}});

  const oldPreview241=window.previewPatchFile240;
  window.previewPatchFile240=async function(){await oldPreview241();if(!pendingPatch19||pendingPatch19.schema!=='studio-os-project-patch-v1')return;const s=guard241(pendingPatch19),host=document.querySelector('#patchPreview19');if(!host)return;host.insertAdjacentHTML('afterbegin',`<div class="version-guard-241"><strong>Version Guard · ${esc(s.action)}</strong><span>${esc(s.reason)}</span></div>`);const btn=[...host.querySelectorAll('button')].find(x=>x.textContent.includes('Merge'));if(['BLOCKED','SKIPPED'].includes(s.action)){if(btn){btn.disabled=true;btn.textContent=s.action==='BLOCKED'?'구버전 차단됨':'동일 Patch 건너뜀'}}else if(btn){btn.onclick=()=>window.applyPatch240();btn.textContent=s.action==='RECOVERY'?'최신 상태 복구':'Merge 적용'}};

  window.applyPatch240=function(){const p=pendingPatch19;if(!p||p.schema!=='studio-os-project-patch-v1')return;const s=guard241(p);if(['BLOCKED','SKIPPED'].includes(s.action)){logGuard241(p,s);data.patchImports.unshift(historyRow241(p,s));saveData();renderExperience19();toast(s.action==='BLOCKED'?`구버전 Patch 차단 · ${s.reason}`:'동일 Patch를 건너뛰었습니다.');return}try{const incoming=clone241(p.project||{});let local=findLocal241(incoming);if(local){const id=local.id;Object.assign(local,incoming,{id})}else{local=incoming;data.projects.push(local)}const localId=local.id;if(p.constitution){const pc=clone241(p.constitution);pc.projectId=localId;upsertV19(data.projectConstitutions,pc,'projectId')}const ruleResult=mergeList241(data.constitution,p.rules||[]),decResult=mergeList241(data.decisions,p.decisions||[]),expResult=mergeList241(data.experiences,p.experiences||[]),assetResult={added:0,updated:0,merged:0,blocked:0};(p.assets||[]).forEach(a=>{const r=mergeAsset241(a,p.projectVersion);assetResult[r]=(assetResult[r]||0)+1});if(p.development){const dIn=clone241(p.development);dIn.projectId=localId;let d=data.developmentRecords.find(x=>x.projectId===localId);if(d)Object.assign(d,dIn);else data.developmentRecords.push(dIn)}if(p.workspace)data.workspaces[localId]=clone241(p.workspace);const row=historyRow241(p,s);row.merge={rules:ruleResult,decisions:decResult,assets:assetResult,experiences:expResult};data.patchImports.unshift(row);logGuard241(p,s);saveData();pendingPatch19=null;renderExperience19();toast(`${s.action==='RECOVERY'?'최신 상태 복구':'Patch Merge'} 완료 · ${p.project?.name||''} ${p.projectVersion}`)}catch(e){toast(`Patch 적용 실패: ${e.message}`)}};

  const oldHistory241=renderPatchHistory19;renderPatchHistory19=function(){oldHistory241();const rows=[...document.querySelectorAll('.patch-history-v19 > div')];rows.forEach((el,i)=>{const x=(data.patchImports||[])[i];if(!x?.status)return;const p=el.querySelector('p');if(p)p.insertAdjacentHTML('beforeend',` · <b>${esc(x.status)}</b>${x.reason?` · ${esc(x.reason)}`:''}`)})};
  const oldCenter241=renderPatchCenter19;renderPatchCenter19=function(){oldCenter241();const p=document.querySelector('.patch-import-v19 p');if(p)p.innerHTML='<code>.studioospatch.json</code> / <code>.dailyclosing.json</code> · Version Guard → Before/After → Safe Merge'};

  data.releaseNotes=data.releaseNotes||[];const rn={id:'RN-2.4.1',version:'v2.4.1',date:'2026-08-09',title:'Version Guard & Safe Import',newItems:['Latest-version-wins Version Guard','구버전 Patch 차단','최신 Patch 복구 Import','프로젝트 이름 우선 매칭'],improved:['Project/Development/Workspace/Asset Safe Merge','Import History 상태 표시'],fixed:['구버전 Patch 롤백','우리집캐디 legacy seed 상태 덮어쓰기'],removed:[],experiences:['EXP-063']};const ri=data.releaseNotes.findIndex(x=>x.id===rn.id);if(ri>=0)data.releaseNotes[ri]=rn;else data.releaseNotes.unshift(rn);saveData();
  const baseRoadmap241=renderRoadmap;renderRoadmap=function(){baseRoadmap241();const rows=document.querySelector('.roadmap-line');if(rows&&!rows.textContent.includes('v2.4.1'))rows.insertAdjacentHTML('beforeend','<div class="roadmap-row current-roadmap"><strong>v2.4.1 · Version Guard & Safe Import — 현재</strong><p>Latest-version-wins · rollback protection · safe recovery</p></div>')};pages.roadmap=renderRoadmap;
  const baseSystem241=pages.system;pages.system=function(){baseSystem241();const rows=[...document.querySelectorAll('.system-row-222')];const version=rows.find(x=>x.textContent.includes('Version'));if(version)version.innerHTML='<div><h3>Version</h3><p>Version Guard & Safe Import</p></div><div class="system-value-222"><strong>Studio OS v2.4.1</strong></div>'};
  document.title='Studio OS v2.4.1 · Version Guard & Safe Import';const brand241=document.querySelector('.brand small');if(brand241)brand241.textContent='Version Guard · v2.4.1';buildNav();
})();
